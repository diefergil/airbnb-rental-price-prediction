...
* [Weight & Biases project](https://wandb.ai/diefergil/nyc_airbnb)
* [Github Project](https://github.com/diefergil/nd0821-c2-build-model-workflow-starter)
...
```
git clone https://github.com/diefergil/airbnb-rental-price-prediction.git
```

After that step into the `airbnb-rental-price-prediction` folder.
...
which are "download", "basic_cleaning", "data_check", "data_split", "train_random_forest" and "test_regression_model" (described in [`main.py`](https://github.com/diefergil/airbnb-rental-price-prediction/blob/master/main.py)). So for example to run the "download" step use:
...
You can also customize each step call. Steps are controlled by the [`hydra`](https://hydra.cc/) package, which is a framework to configure complex applications. The customizable pipeline parameters are contained in the [`config.yaml`](https://github.com/diefergil/airbnb-rental-price-prediction/blob/master/config.yaml) file. 
...
according to the way specified in the [`config.yaml`](https://github.com/diefergil/airbnb-rental-price-prediction/blob/master/config.yaml) file. Another example is:
...
In this repository there's also a [release](https://github.com/diefergil/airbnb-rental-price-prediction/releases) version of the pipeline. You can can use the release to automatically run the training pipeline without cloning the repo, just by having followed the instructions in the setup. 

To run the latest release at thist points version 1.0.1) from github use in a different sample dataset use:

```
mlflow run -v 1.0.1 https://github.com/diefergil/airbnb-rental-price-prediction.git -P hydra_options="etl.sample='sample2.csv'"
```
...
After the `train_random_forest` (specially if you use multiple runs, as described in the Multiple Runs section), you must go to weights and biases and tag a run. 

[Best experiment](https://wandb.ai/diefergil/nyc_airbnb/runs/300znjex/overview)
[Table with experiments](https://wandb.ai/diefergil/nyc_airbnb/artifacts/model_export/random_forest_export)
...
- [weights and biases public project](https://wandb.ai/diefergil/nyc_airbnb/)
- [github link](https://github.com/diefergil/airbnb-rental-price-prediction/)

# ======== INSTRUCTIONS ======== 
# Build an ML Pipeline for Short-Term Rental Prices in NYC (Exercise)
You are working for a property management company renting rooms and properties for short periods of 
time on various rental platforms. You need to estimate the typical price for a given property based 
on the price of similar properties. Your company receives new data in bulk every week. The model needs 
to be retrained with the same cadence, necessitating an end-to-end pipeline that can be reused.
